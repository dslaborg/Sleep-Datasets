...
def compute_soft_agreement(hypnogram, hypnograms_consensus):
    ...
    for hypnogram_consensus in hypnograms_consensus:
        probabilistic_consensus[
            np.array(hypnogram_consensus) + 1, range(len(hypnogram))
        ] += 1
    probabilistic_consensus_normalized = (
            probabilistic_consensus / probabilistic_consensus.max(0)
    )
    soft_agreement = probabilistic_consensus_normalized[
        np.array(hypnogram) + 1, epochs
    ].mean()
...
def build_consensus_hypnogram(ranked_hypnograms):
    ...
    for hypnogram_consensus in ranked_hypnograms_consensus_array:
        probabilistic_consensus[
            np.array(hypnogram_consensus), range(number_of_epochs)
        ] += 1
    ...
    # [0, 1, 1, 2, 2], then we have a tie and would use the frist score (0) as the resolution, although the tie is
    # between the scores 1 and 2).
    for tie_epoch in np.where(ties)[0]:
        classes = np.where(
            probabilistic_consensus[:, tie_epoch]
            == np.max(probabilistic_consensus[:, tie_epoch])
        )[0]
        consensus_hypnogram[tie_epoch] = [
            score
            for score in ranked_hypnograms_consensus_array[:, tie_epoch]
            if score in classes
        ][0]

    consensus_probability = probabilistic_consensus[
                                consensus_hypnogram, range(number_of_epochs)
...
class ConsensusBuilder:
    def __init__(
        ...
        self.index = {}
        self.scorers_folder = {
            scorer: f"{scorers_folder}{scorer}/" for scorer in self.scorers
        }
            ...
            list(
                set.intersection(
                    *(
                        {
                            record.split(".json")[0]
                            for record in os.listdir(self.scorers_folder[scorer])
                            if record.split(".json")[0] not in record_blacklist
                        }
        ...
        for record in self.records:
            ...
            hypnograms = [
                self.scorer_hypnograms[scorer][record] for scorer in self.scorers
            ]
            index_min = max(
                [np.where(np.array(hypnogram) >= 0)[0][0] for hypnogram in hypnograms]
            )  # NG: remove continuous artifacts at the start of recording
            ...
            index_max = len(hypnograms[0]) - max(
                [
                    np.where(np.array(hypnogram)[::-1] >= 0)[0][0]
            ...
            for scorer in self.scorers:
                ...
                self.scorer_hypnograms[scorer][record] = self.scorer_hypnograms[scorer][
                    record
                ][index_min:index_max]
                self.hypnogram_sizes[record] += (
                        index_max - self.hypnogram_sizes[record] - index_min
                )
            ...
            record: sorted(
                ...
                key=lambda scorer: -compute_soft_agreement(
                    ...
                    [
                        ...
                        for other_scorer in self.scorers
        ...
        self.scorer_hypnograms_consensus = {
            scorer: {
                record: build_consensus_hypnogram(
                    [
                        self.scorer_hypnograms[other_scorer][record]
                        for other_scorer in self.scorers_ranking[record]
                        if other_scorer != scorer
                    ]
                )
                for record in self.records
        ...
        self.result_hypnograms_consensus = {
            record: build_consensus_hypnogram(
                [
                    self.scorer_hypnograms[scorer][record]
                    for scorer in self.scorers_ranking[record][:]
                ]  # all scorings
            )
            for record in self.records
        }
        ...
        self.result_hypnograms_consensus_n_minus_one = {
            record: build_consensus_hypnogram(
                [
                    self.scorer_hypnograms[scorer][record]
                    for scorer in self.scorers_ranking[record][:-1]
                ]  # N - 1 scorings
            )
            for record in self.records
        }
